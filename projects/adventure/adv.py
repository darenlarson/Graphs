...
# ============== GLOBAL VARIABLES ===================
# Keeps track of all movements player makes
traversalPath = []
# Keeps track of how directions for player to move backwards to find a room that has unexplored exits
escapePath = []
...
print("Initial graphMap:", graphMap)
# ============== GLOBAL VARIABLES ===================


# Primary function used to traverse the world of rooms
def world_traversal():
    print("world_traversal() invoked")
    # Initialize unexplored to True so we can start the while loop
    all_rooms_explored = False

    # While unexplored_world_check() is setting unexplored = True
    while all_rooms_explored is False:
        print("----- START OUTERMOST WHILE LOOP -----")
        print("****CURRENT ROOM:", player.currentRoom.id)
        print("traversalPath:", traversalPath)
        print("escapePath:", escapePath)
        
        # Check to see if there are any unexplored exits in the current room
        all_exits_explored = unexplored_exit_check(player.currentRoom)
        print("all_exits_explored:", all_exits_explored)

        # While there are no unexplored exits in current room, we'll go back using go_back()
        while all_exits_explored is True:
            go_back(player.currentRoom)
            all_exits_explored = unexplored_exit_check(player.currentRoom)
            print("all_exits_explored:", all_exits_explored)
            


        # While there are unexplored exits in current room, we'll go forward using go_forward()
        while all_exits_explored is False:
            go_forward(player.currentRoom.id)
            # Check to see if there are any unexplored exits in the current room
            all_exits_explored = unexplored_exit_check(player.currentRoom)
            print("all_exits_explored:", all_exits_explored)


        all_rooms_explored = unexplored_world_check()
...
def go_forward(cur_room_id):
    ...
    # Save the room in a temp variable so we still have access to it
    old_room_id = cur_room_id

    # Pick a direction for the player to travel in
    for direction in ['n','e','s','w']:
        if direction in graphMap[cur_room_id] and graphMap[cur_room_id][direction] == '?':
            
            # Make the player travel in the chosen direction
            player.travel(direction)

            # Update the traversal path and escape path
            traversalPath.append(direction)
            escapePath.append(opposite_direction(direction))

            # Add new room entered to the graphMap with exits initialized to '?'
            ...
            # Add previous room to the opposite direction we just travelled
            graphMap[player.currentRoom.id][opposite_direction(direction)] = old_room_id

            # Add current room to the previous room's direction we just travelled
            graphMap[old_room_id][direction] = player.currentRoom.id
...
def go_back(cur_room):
    ...
    # while unexplored_exit_check(player.currentRoom.id) == True:
    # Remove the last item in escape path. We're going to move back this direction
    escape_direction = escapePath.pop()

    # Move back one step in the escape_direction
    player.travel(escape_direction)
    
    # Add this movement to traversalPath
    traversalPath.append(escape_direction)

    print("traversalPath:", traversalPath)
    print("escapePath:", escapePath)
    print("****CURRENT ROOM:", player.currentRoom.id)

def unexplored_exit_check(cur_room):
    print("unexplored_exit_check() invoked")
    exits = cur_room.getExits()
    for exit in exits:
        if graphMap[player.currentRoom.id][exit] == '?':
...
def unexplored_world_check():
    print("unexplored_world_check() invoked")
    for key in graphMap:
        print(f"graphMap[{key}]: {graphMap[key]}")
        for exit_direction, room in graphMap[key].items():
            print(f"graphMap[{key}] exit: {exit_direction}: {room}")
            if room == '?':
                return False
    return True
